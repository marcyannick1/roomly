...
async def get_landlord_received_likes(
    ...
    result = await db.execute(
        ...
        .options(
            selectinload(Like.student).selectinload(Student.user),
            selectinload(Like.listing).selectinload(Listing.photos)
        )
    )
    likes = result.scalars().all()
    
    # Formater les données pour inclure les infos de l'utilisateur et du match
    formatted_likes = []
    for like in likes:
        # Vérifier s'il existe un match pour ce like
        ...
        # Récupérer les infos du landlord si match existe
        matched_landlord = None
        if match:
        ...
        like_dict = {
            ...
            "student": {
                "id": like.student.id,
                "user_id": like.student.user_id,
                "university": like.student.university,
                "study_level": like.student.study_level,
                "name": like.student.user.name if like.student.user else None,
                "photo": like.student.user.photo if like.student.user else None,
            },
            "listing": like.listing,
            "match": {
                "exists": match is not None,
                "match_id": match.id if match else None,
                "landlord": matched_landlord
            }
        }
        formatted_likes.append(like_dict)
    
    return formatted_likes


async def has_student_liked_listing(
    db: AsyncSession,
    student_id: int,
    listing_id: int
) -> bool:
    """Vérifier si un étudiant a liké une annonce"""
    result = await db.execute(
        select(Like).where(
